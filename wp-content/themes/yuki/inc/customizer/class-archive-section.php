...
if ( !class_exists( 'Yuki_Archive_Section' ) ) {
    class Yuki_Archive_Section extends CustomizeSection
    {
        ...
        protected function getArchiveLayoutControls()
        {
            ...
            ] )->setControls( $this->getArchiveColumnsControls() )->setReverseControls( [ ( new Slider( 'yuki_archive_image_width' ) )->setLabel( __( 'Thumbnail Width', 'yuki' ) )->setDefaultUnit( '%' )->setMin( 0 )->setMax( 100 )->enableResponsive()->setDefaultValue( [
                'desktop' => '45%',
                'tablet'  => '45%',
                'mobile'  => '100%',
            ] ) ] ), ( new Slider( 'yuki_card_gap' ) )->setLabel( __( 'Card Gap', 'yuki' ) )->enableResponsive()->setDefaultUnit( 'px' )->setDefaultValue( '24px' ) ];
        }
        ...
        {
            $controls = [
                ( new Radio( 'yuki_archive_header_alignment' ) )->setLabel( __( 'Alignment', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->buttonsGroupView()->setDefaultValue( 'left' )->setChoices( [
                'left'   => __( 'Left', 'yuki' ),
                'center' => __( 'Center', 'yuki' ),
                'right'  => __( 'Right', 'yuki' ),
            ] ),
                new Separator(),
                ( new Spacing( 'yuki_archive_header_padding' ) )->setLabel( __( 'Padding', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->setSpacing( [
                'top'    => '24px',
                'bottom' => '24px',
                'left'   => '24px',
                'right'  => '24px',
        ...
        protected function getCardControls()
        {
            return [ ( new Tabs() )->setActiveTab( 'content' )->addTab( 'content', __( 'Content', 'yuki' ), array_merge( [ $this->getPostElementsLayer( 'yuki_card_structure', 'entry', [
                ...
                'tags'  => [],
                'metas' => [],
            ] ), new Separator() ], $this->getCardContentControls( 'yuki_' ) ) )->addTab( 'style', __( 'Style', 'yuki' ), $this->getCardStyleControls( 'yuki_', [
                'shadow' => [
                'rgba(44, 62, 80, 0.25)',
                '0px',
                '15px',
        ...
        protected function getPaginationContentControls()
        {
            ...
            $controls[] = ( new ImageRadio( 'yuki_pagination_alignment' ) )->setLabel( __( 'Alignment', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->inlineChoices()->setDefaultValue( 'flex-start' )->setChoices( [
                'flex-start' => [
        ...
        {
            $controls = [ ( new Typography( 'yuki_pagination_typography' ) )->setLabel( __( 'Typography', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->setDefaultValue( [
                'family'     => 'inherit',
                'fontSize'   => '0.875rem',
                'variant'    => '400',
                'lineHeight' => '1',
                ...
                'yuki_pagination_type' => 'numbered|prev-next',
            ] )->setControls( [ ( new ColorPicker( 'yuki_pagination_button_color' ) )->setLabel( __( 'Text Color', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->addColor( 'initial', __( 'Text Initial', 'yuki' ), 'var(--yuki-accent-active)' )->addColor( 'active', __( 'Text Active', 'yuki' ), 'var(--yuki-base-color)' )->addColor( 'accent', __( 'Accent', 'yuki' ), 'var(--yuki-primary-color)' ), ( new Border( 'yuki_pagination_button_border' ) )->setLabel( __( 'Border', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->displayInline()->setDefaultBorder( 1, 'solid', 'var(--yuki-base-200)' ) ] ) ];
            $controls[] = ( new Condition() )->setCondition( [
                'yuki_pagination_type' => 'numbered|prev-next|load-more',
            ] )->setControls( [ new Separator(), ( new Slider( 'yuki_pagination_button_radius' ) )->setLabel( __( 'Radius', 'yuki' ) )->enableResponsive()->bindSelectiveRefresh( 'yuki-dynamic-css' )->setDefaultUnit( 'px' )->setDefaultValue( '4px' )->setMin( 0 )->setMax( 100 ) ] );
            return $controls;
        }
