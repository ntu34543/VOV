	...
	class Yuki_Reviews_Element extends Element {
		...
		public function enqueue_frontend_scripts( $id = null, $data = [] ) {
			...
			add_filter( 'yuki_filter_dynamic_css', function ( $css ) use ( $id, $settings ) {
				...
				$css[".yuki-reviews.$id .review-rating"] = Css::colors( $this->get( $this->getSlug( 'card-color' ) ), [
					'rating' => 'color',
				] );

				$css[".yuki-reviews.$id .review-message"] = array_merge(
					Css::colors( $this->get( $this->getSlug( 'card-color' ) ), [
						'message' => 'color',
					] ),
					Css::typography( $this->get( $this->getSlug( 'message-typography' ) ) )
				);
				...
				$css[".yuki-reviews.$id .review-reviewer"] = array_merge(
					...
					Css::typography( $this->get( $this->getSlug( 'reviewer-typography' ) ) )
				);
				...
				$css[".yuki-reviews.$id .review-job-title"] = array_merge(
					...
					Css::typography( $this->get( $this->getSlug( 'job-title-typography' ) ) )
				);
