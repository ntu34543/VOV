    ...
    class Yuki_Logo_Element extends Element
    {
        ...
        public function getControls()
        {
            return [ ( new Tabs() )->setActiveTab( 'content' )->addTab( 'content', __( 'Content', 'yuki' ), [
                ...
                ( new Slider( $this->getSlug( 'height' ) ) )->setLabel( __( 'Logo Height', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->setMin( 0 )->setMax( 300 )->setDefaultUnit( 'px' )->setDefaultValue( $this->getDefaultSetting( 'height', '40px' ) ),
                ( new Slider( $this->getSlug( 'spacing' ) ) )->setLabel( __( 'Logo Spacing', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->setMin( 0 )->setMax( 300 )->setDefaultUnit( 'px' )->setDefaultValue( $this->getDefaultSetting( 'spacing', '12px' ) ),
                ( new Separator() )->dashedStyle(),
                ( new Toggle( $this->getSlug( 'enable_site_title' ) ) )->setLabel( __( 'Site Title', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->selectiveRefresh( ...$this->selectiveRefresh() )->setDefaultValue( $this->getDefaultSetting( 'enable-site-title', 'yes' ) ),
                ( new Toggle( $this->getSlug( 'enable_site_tagline' ) ) )->setLabel( __( 'Site Tagline', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->selectiveRefresh( ...$this->selectiveRefresh() )->setDefaultValue( $this->getDefaultSetting( 'enable-site-tagline', 'no' ) ),
                ( new CallToAction() )->setLabel( __( 'Click here to change the site title and tagline.', 'yuki' ) )->expandCustomize( 'title_tagline' ),
                ( new Separator() )->dashedStyle(),
                ( new Radio( $this->getSlug( 'content_alignment' ) ) )->setLabel( __( 'Content Alignment', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->buttonsGroupView()->setDefaultValue( $this->getDefaultSetting( 'content-alignment', 'left' ) )->setChoices( [
                'left'   => __( 'Left', 'yuki' ),
