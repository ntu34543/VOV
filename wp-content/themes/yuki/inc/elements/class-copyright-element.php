...
if ( !class_exists( 'Yuki_Copyright_Element' ) ) {
    class Yuki_Copyright_Element extends Element
    {
        ...
        {
            return [ ( new Tabs() )->setActiveTab( 'content' )->addTab( 'content', __( 'Content', 'yuki' ), $this->getContentControls() )->addTab( 'style', __( 'Style', 'yuki' ), [ ( new Typography( $this->getSlug( 'typography' ) ) )->setLabel( __( 'Typography', 'yuki' ) )->bindSelectiveRefresh( 'yuki-dynamic-css' )->setDefaultValue( [
                'family'     => 'inherit',
                'fontSize'   => '0.85rem',
                'variant'    => '400',
                'lineHeight' => '1.5em',
            ] ), ( new ColorPicker( $this->getSlug( 'color' ) ) )->setLabel( __( 'Color', 'yuki' ) )->enableAlpha()->bindSelectiveRefresh( 'yuki-dynamic-css' )->addColor( 'text', __( 'Text Initial', 'yuki' ), 'var(--yuki-accent-active)' )->addColor( 'initial', __( 'Initial', 'yuki' ), 'var(--yuki-primary-color)' )->addColor( 'hover', __( 'Hover', 'yuki' ), 'var(--yuki-primary-active)' ) ] ) ];
        }
