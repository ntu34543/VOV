		...
		public function getControls() {
			return [
				( new Tabs() )
					...
					->addTab(
						'style', __( 'Style', 'yuki' ),
						$this->getButtonStyleControls( $this->slug . '_', [ 'selective-refresh' => true ] )
					),
			];
		}
		...
		public function enqueue_frontend_scripts() {
			...
			add_filter( 'yuki_filter_dynamic_css', function ( $css ) {
				...
				$css[".{$this->slug}"] = array_merge(
					...
					Css::shadow( CZ::get( $this->getSlug( 'shadow' ) ) ),
				...
				$css[".{$this->slug}:hover"] = Css::shadow( CZ::get( $this->getSlug( 'shadow_active' ) ) );
